...
package io.apicurio.designer.common.config;
...
public class PropertiesUtil {
     ...
     *
     * @param ip the injection point annotated with {@link DesignerProperties}
     * @return filtered/stripped properties
     */
    public static Properties properties(InjectionPoint ip) {
        DesignerProperties cp = ip.getAnnotated().getAnnotation(DesignerProperties.class);
        if (cp == null) {
            throw new IllegalArgumentException(
                    ip.getMember() + " is not annotated with @DesignerProperties"
            );
        }
        ...
        if (prefixes.length == 0) {
            throw new IllegalArgumentException(
                    "Annotation @DesignerProperties on " + ip.getMember() +
